...
Provide specific, actionable feedback to help the candidate improve.`;
  ...
  private static validateFeedback(feedback: any): string {
  ...
  private static validateMissingPoints(missingPoints: any, expectedPoints: string[]): string[] {
  ...
  private static createFallbackResponse(
    ...
    return {
      score,
      feedback: content.length > 0 ? content.substring(0, 500) : 'Could not evaluate answer properly.',
      missing_points: expectedPoints,
      suggestions: ['Try explaining your approach more clearly', 'Include time and space complexity analysis']
    };
  }
